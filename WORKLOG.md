...
Tests: new isolation tests 17/17 (13 failed on pre-fix code with 200/201 instead of 403/404; the 4 legitimate-flow tests pass before and after). Targeted run (15 files touching the changed endpoints): 193 passed / 8 failed, all 8 the known pre-existing e2e quote-approval chain. Broad run (18 files: supplier/BOQ/MR/warehouse/site-dashboard/security/attachments plus every procurement suite): 277 passed / 12 failed. Of those, 8 are known pre-existing (MR auto-close, `TestQuoteApprove`) and 4 are `test_procurement_analytics` Excel exports failing because `openpyxl` is missing from the local venv (a local environment gap, not a code change). Frontend unchanged; `tsc` clean, `vite build` passes, Playwright 32/32.
Verdict: Self-verified; no independent verifier agent run.
Unresolved: production data mismatch pre-check and `stock_ledger.created_at` default check (KNOWN_BUGS); requester self-vote governance decision; quote-approval/MR auto-close test failures (separate, pre-existing). Full-application isolation audit not performed; only the routes named here were audited.

## 2026-09-15 — Site Project Transfer via vote-based request; transfer destination authorization
Changed: `hmh-frontend/src/pages/SiteDashboardPage.tsx` (transfer modal → `warehouseTransfersApi.submitRequest`, reason required, "Pending office approval" confirmation); `hmh-backend/app/api/v1/warehouse_transfers.py` (destination project access on submit); `app/api/v1/warehouse.py` (`main/transfer-to-site` destination project access); `app/api/v1/stock.py` (`issue-to-lot`, `site-transfer` → `OFFICE_AND_ABOVE`); `app/services/warehouse_transfer_service.py` (`_execute_transfer` writes `created_at`); tests `test_warehouse_transfer_requests.py` (new), `test_site_clerk_stock_permissions.py` (+2 routes), Playwright spec (+1 test).
Behaviour: A Site Clerk submits a PENDING transfer request and no stock moves until 3 office votes or an owner override. Site roles get 403 on vote/override/reject, on the direct transfer route, on requests or global transfers into inaccessible projects, and on issue-to-lot/site-transfer. Approved transfers now execute on databases with no `created_at` default (they previously failed with NOT NULL).
Tests: transfer requests 12/12 (4 failed on pre-fix code); permission tests' 2 site-role cases failed on pre-fix code, pass now. Playwright 32/32 (the new transfer test failed on the old modal). `tsc` clean, `vite build` passes. Backend regression (23 files): 317 passed / 16 failed, the same 16 already confirmed pre-existing (quote approval / pipeline auto-close).
Verdict: Self-verified; no independent verifier agent run.
Unresolved: project-isolation gaps on usage, receive-with-document, site-capture and transfer GET-by-id; requester self-vote (KNOWN_BUGS). Production `stock_ledger.created_at` default Unknown.

## 2026-09-15 — Client review: site MR supplier picker, remove Add to Warehouse, BOQ Add item supplier
Changed: `hmh-backend/app/api/v1/warehouse.py` (main-warehouse receive/adjust/add-tool → `OFFICE_AND_ABOVE`); `hmh-backend/app/services/mr_service.py` (`_validate_requested_suppliers`); `hmh-frontend/src/pages/SiteDashboardPage.tsx` (per-line Supplier dropdown; Add to Warehouse action/modal removed; Remove stock hidden for site roles); `hmh-frontend/src/pages/BOQBuilderPage.tsx` (Add item: Specification + shared `SupplierSelect`); `hmh-frontend/package.json` (`test:site-boq`); tests `test_mr_requested_supplier.py`, `test_site_clerk_stock_permissions.py`, `test_boq_add_item_supplier.py`, `tests/site-requests-warehouse-boq.spec.mjs`.
Behaviour: Site Clerk sees the BOQ default supplier preselected and can pick another active supplier per line. It is stored on the MR line and header, and the BOQ default is untouched. Unknown supplier ids are now 422 (previously a 500 from a foreign-key error), and inactive ids are 422. Site roles get 403 on every manual stock add/write-off route. Admin Add item saves supplier and specification in one create call. No schema change.
Tests: new backend 25/25 (4 failed on original code for the right reason); regression run 296 passed / 16 failed, all 16 confirmed pre-existing via stash. Playwright 31/31 (7 new; 4 of them failed on the original UI build). `tsc` clean; `vite build` passes.
Verdict: Self-verified; no independent verifier agent run.
Unresolved: site "Project Transfer" 403 for site roles, offline drafts drop supplier, and quote-approval test failures (all in KNOWN_BUGS). ESLint not installed/configured in hmh-frontend. No deploy, no production data touched.
