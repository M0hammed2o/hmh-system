...
-- PostgreSQL 15+ Schema — V1 Finalized
-- =============================================================================
-- This is the single source of truth for the database schema.
-- Used by Alembic models and as reference for all migrations.
-- =============================================================================
...
CREATE TYPE movement_type_enum AS ENUM (
    ...
    'ADJUSTMENT_SUBTRACT',
    'RETURN_TO_STORE',
    'TRANSFER_IN',     -- reserved for V2; do not use in V1 business logic
    'TRANSFER_OUT'     -- reserved for V2; do not use in V1 business logic
);

CREATE TYPE invoice_match_status_enum AS ENUM (
    'MATCHED',
    'PARTIALLY_MATCHED',
    'MISMATCH',
    'UNLINKED'
...
CREATE TYPE opening_financial_status_enum AS ENUM (
    'OUTSTANDING',
    'PARTIALLY_PAID',
    'PAID',
    'DISPUTED'
...
CREATE TYPE opening_reference_type_enum AS ENUM (
    'INVOICE',
    'PO',
    'PAYMENT',
    'OTHER'
...
-- Standard V1 stage seed (applied by scripts/seed_stages.py)
-- Platform, Slab, Wallplate, Roof, Completion,
-- Plumbing, Paint, Tank, Apron, Screed, Beam Filling
...
CREATE TABLE project_stage_status (
    ...
    created_at               TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at               TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    -- PostgreSQL 15+: NULLs are treated as equal in this unique constraint
    UNIQUE NULLS NOT DISTINCT (project_id, site_id, lot_id, stage_id)
);
...
CREATE TABLE suppliers (
    ...
    is_active      BOOLEAN     NOT NULL DEFAULT TRUE,
    created_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at     TIMESTAMPTZ NOT NULL DEFAULT NOW()   -- V1 fix: added updated_at
);
...
CREATE TABLE item_aliases (
    ...
    -- V1 fix: alias_name is globally unique so one alias maps to exactly one item
...
-- V1 fix: only one active version per project enforced at DB level
...
CREATE TABLE boq_items (
    ...
    -- V1 fix: planned_total is a generated column (computed from qty * rate)
    planned_total           NUMERIC(14,2) GENERATED ALWAYS AS (
                                CASE
                                    WHEN planned_quantity IS NOT NULL AND planned_rate IS NOT NULL
...
CREATE TABLE purchase_orders (
    ...
    -- V1: totals stored on PO; updated by application on item add/edit
...
-- =============================================================================
-- PURCHASE ORDER TOTALS TRIGGER
-- V1 assumes supplier pricing is VAT-inclusive. vat_amount is a reporting
-- breakdown derived from subtotal_amount, and total_amount equals subtotal_amount.
-- =============================================================================
...
BEGIN
    ...
    v_vat := ROUND(v_subtotal * 15 / 115, 2);
...
CREATE TRIGGER trg_po_items_recalculate_totals
AFTER INSERT OR UPDATE OR DELETE ON purchase_order_items
FOR EACH ROW EXECUTE FUNCTION fn_recalculate_po_totals();
...
CREATE TABLE deliveries (
    id                            UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    -- V1 fix: delivery_number unique per project when not null
...
-- Delivery number unique per project (NULLs allowed, duplicates not)
CREATE UNIQUE INDEX idx_deliveries_number_per_project
    ON deliveries(project_id, delivery_number)
...
-- =============================================================================
-- STOCK LEDGER  (immutable — never update, only insert)
-- =============================================================================
...
-- V1 fix: materialized view for stock balances (refreshed after each ledger insert)
...
CREATE UNIQUE INDEX idx_stock_balances_pk
    ON stock_balances(project_id, site_id, COALESCE(lot_id, '00000000-0000-0000-0000-000000000000'::uuid), item_id);

CREATE INDEX idx_stock_balances_item ON stock_balances(item_id);

-- =============================================================================
-- USAGE LOGS
-- V1 fix: remaining_quantity_after_use removed — derive from stock_balances view
-- =============================================================================
...
CREATE TABLE usage_logs (
    ...
    -- No remaining_quantity_after_use: use stock_balances materialized view
...
-- V1 fix: payment_reference unique per project (NULLs allowed, duplicates not)
CREATE UNIQUE INDEX idx_payments_reference_per_project
...
CREATE TABLE system_alerts (
    ...
    -- V1 fix: target_role and target_user_id added for directed alerts
    target_role          user_role_enum,
    target_user_id       UUID        REFERENCES users(id) ON DELETE SET NULL,
    notification_channel VARCHAR(50) NOT NULL DEFAULT 'in_app',
    sent_at              TIMESTAMPTZ,
    read_at              TIMESTAMPTZ,
    -- V1 fix: acknowledged_by added
...
CREATE TABLE project_opening_balances (
    ...
    entered_by       UUID        REFERENCES users(id) ON DELETE SET NULL,
    -- V1 fix: track whether opening balance has been posted to the stock ledger
...
-- =============================================================================
-- SYSTEM CONFIG  (alert thresholds — managed by developers, not by UI in V1)
-- =============================================================================
...
INSERT INTO system_config (config_key, config_value, description) VALUES
    ('boq_variance_overuse_threshold_pct', '10',
     'Percentage over BOQ planned quantity that triggers BOQ_VARIANCE_OVERUSE alert'),
    ('low_stock_threshold_pct',            '20',
     'Percentage of planned quantity remaining that triggers LOW_STOCK alert'),
    ('overdue_payment_days',               '30',
     'Days past invoice due_date before OVERDUE_PAYMENT alert fires'),
    ('request_pending_too_long_days',      '5',
     'Days a material request stays in SUBMITTED before REQUEST_PENDING_TOO_LONG alert fires');
